# 아기상어(BFS)
# 아기상어는 먹을 수 있는 물고기 중에서 가장 가까운 물고기를 먼저 먹는다
# 가장 가까운 물고기(최단 거리 알고리즘)(BFS)
...
# 아기 상어의 현재 크기 변수, 현재 위치 변수
now_size = 2
now_x, now_y = 0, 0

# 아기 상어의 시작위치를 찾은 뒤에 그 위치엔 아무것도 없다고 처리
for i in range(n):
    for j in range(n):
        if array[i][j] == 9:
            now_x, now_y = i, j
...
def find(dist):
    ...
    if min_dist == INF: # 먹을 수 있는 물고기가 없는 경우
        ...
        return x, y, min_dist   # 먹을 수 있는 물고기가 있는 경우
